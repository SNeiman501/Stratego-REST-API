  ...
  if (playerSlot === 1) {
    ...
    for (let x = 0; x < map.length; x++) {
      for (let y = 4; y < map[x].length; y++) {
        if (map[x][y] !== null) {
          ...
          return {
            succes: false,
            message: "se trato de colocar unidades en una zona invalida",
          };
        }
      }
    }
    if (writtenInValidTerritory === true) {
      ...
      for (let x = 0; x < map.length; x++) {
        for (let y = 0; y < 4; y++) {
          if (playerKey === map[x][y].owner) {
            //verifica si las unidades son del jugador
            ...
            return {
              succes: false,
              message: "se intento colocar unidades que no son del usuario",
            };
          }
        }
      }
      ...
      if (JSON.stringify(unitCounter) === JSON.stringify(startingUnits)) {
        //no se si esta comnparacion funcionara
        return { succes: true, message: "jugada valida" };
      }
    } else {
      return {
        succes: false,
        message: "la cantidad de unidades colocadas es incorrecta",
      };
    }
  }
  if (playerSlot === 2) {
